...
DASHBOARD_DB_PATH = "/tmp/dashboard_test.db"
LOGIN_EMAIL = "admin@test.com"
LOGIN_PASSWORD = "adminpass123"
LOGIN_USERNAME = "admin"

SPEC_CONTENT = (
    "# UI Test Project Spec\n\n"
    "This is the **Bold** spec content with *italic* and `inline code`.\n\n"
    "## Section One\n\n"
    "More content here for the spec doc. It needs to be long enough to be meaningful.\n\n"
    "### Subsection\n\n"
    "- Bullet point one\n- Bullet point two\n\n"
    "**Another bold** statement here."
)


@pytest.fixture(scope="module")
def server():
    """Ensure dashboard server is reachable (started externally on :8090)."""
    for _ in range(20):
        try:
            ...
            time.sleep(0.5)
    pytest.skip("Dashboard server not running on :8090")


@pytest.fixture(scope="module")
def seeded_user(server):
    """Seed the dashboard database with a login user plus the sample project,
    tasks, docs, and agent that the authenticated-UI tests assert against."""
    import db as db_module
    prev = db_module.DB_PATH
    db_module.DB_PATH = DASHBOARD_DB_PATH
    try:
        db_module.init_db()

        # Login user
        db_module.create_user(LOGIN_USERNAME, email=LOGIN_EMAIL, password=LOGIN_PASSWORD)
        with db_module.get_connection() as conn:
            user = conn.execute(
                "SELECT id FROM users WHERE username = ?", (LOGIN_USERNAME,)
            ).fetchone()
        assert user is not None, "failed to seed login user"
        user_id = user["id"]

        # Agent "ui-tester" (admin agents page asserts its presence)
        db_module.create_agent(user_id, "ui-tester", master_name="UI Test Master")

        # Project + tasks with the exact IDs the tests reference
        with db_module.get_connection() as conn:
            conn.execute(
                "DELETE FROM project_docs WHERE project_id = ?", (PROJECT_ID,)
            )
            conn.execute(
                "DELETE FROM comments WHERE entity_id IN (?, ?, ?)",
                (PROJECT_ID, PARENT_TASK_ID, CHILD_TASK_ID),
            )
            conn.execute(
                "DELETE FROM tasks WHERE id IN (?, ?)", (CHILD_TASK_ID, PARENT_TASK_ID)
            )
            ...
            conn.execute(
                ...
                (
                    PROJECT_ID,
                    "UI Test Project",
                    "A test project for UI dashboard integration tests spanning 40+ chars",
                    user_id,
                ),
            )
            conn.execute(
                "INSERT INTO tasks (id, project_id, title, description, status, rank) "
                "VALUES (?, ?, ?, ?, 'pending', ?)",
                (PARENT_TASK_ID, PROJECT_ID, "Parent Task", "Parent task for testing UI dashboard", 0),
            )
            conn.execute(
                "INSERT INTO tasks (id, project_id, parent_id, title, description, status, rank) "
                "VALUES (?, ?, ?, ?, ?, 'pending', ?)",
                (CHILD_TASK_ID, PROJECT_ID, PARENT_TASK_ID, "Child Task", "Child task for testing UI dashboard", 1),
            )
            conn.execute(
                "INSERT INTO project_docs (id, project_id, doc_type, content) "
                "VALUES (?, ?, 'spec', ?)",
                (str(uuid.uuid4()), PROJECT_ID, SPEC_CONTENT),
            )
    finally:
...
class TestMutations:
    ...
    def test_flash_cookie_on_task_create_redirect(self, client, seeded_user):
        login = client.post("/login", data={"email": LOGIN_EMAIL, "password": LOGIN_PASSWORD})
